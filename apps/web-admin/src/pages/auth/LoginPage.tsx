...
export function LoginPage() {
  ...
  const [email, setEmail] = useState("admin@foodo.local");
  ...
  return (
    <section className="animate-fade-in mx-auto max-w-md rounded-3xl border border-white/70 bg-white/85 p-6 shadow-float backdrop-blur-sm sm:p-7">
      <div className="mb-5">
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-brand-700">Admin Access</p>
        <h2 className="mt-2 text-2xl font-extrabold tracking-tight text-slate-900">Admin Sign in</h2>
        <p className="mt-1 text-sm text-slate-600">Sign in with admin credentials.</p>
      </div>
      ...
      <p className="mt-4 text-xs text-slate-500">Demo account: admin@foodo.local / admin123</p>
    </section>
  );
}
